...
int main(){
    ...
    boleto_trabajador=boleto*40/100;
    if(estudiantes==0 || trabajadores==0)
            total_recaudado=boletos_vendidos*boleto;
    if(estudiantes!=0 || trabajadores!=0)
            total_recaudado=(boletos_vendidos*boleto)-(trabajadores*boleto_trabajador)-(estudiantes*boleto_estudiantil);
    std::cout<<"El total recaudado es la suma de: $"<<total_recaudado<<std::endl;
    return 0;
}
